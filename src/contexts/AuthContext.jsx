      ...
      if (user) {
        ...
        try {
          ...
          if (userSnap.exists()) {
            ...
            console.log(
              "User not found in any account collection, setting default citizen role"
            );
          ...
          // Set default citizen role in case of error
          setUserData({ role: "citizen" });
        }
      } else {
        setUserData(null);
  ...
  const loginWithEmail = async (email, password) => {
    try {
      ...
      if (userSnap.exists()) {
        ...
        console.log(
          "Found user in users collection, letting onAuthStateChanged handle userData"
        );
      } else {
        // Don't set userData here - let onAuthStateChanged handle it
        console.log(
          "User not found in users collection, letting onAuthStateChanged handle userData"
        );
      }
